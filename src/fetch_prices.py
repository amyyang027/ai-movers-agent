...
def get_close_near(ticker: str, target_date: dt.date, max_lookback_days: int = 5):
    """Return (date_used, close_price) for the most recent trading day on or before
    target_date. Returns (None, None) if nothing is found within max_lookback_days
    (e.g. ticker delisted, bad symbol, extended holiday)."""
